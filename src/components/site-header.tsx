...
import { Book, LogOut, Shield, Search, Menu, X } from "lucide-react";
    ...
    <header className="sticky top-0 z-40 border-b border-border/60 glass">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          {loading ? null : isAdmin && user ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="rounded-full">
                  <Shield className="h-4 w-4" />
                  <span className="hidden sm:inline ml-1.5 max-w-32 truncate">Admin</span>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem asChild>
                  <Link to="/admin/catalogs">Manage catalogs</Link>
                </DropdownMenuItem>
                <DropdownMenuSeparator />
            ...
            <Button asChild size="sm" variant="outline" className="rounded-full hidden sm:flex">
